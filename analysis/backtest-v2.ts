 ...
 * sellers size positions; they post margin, not full notional. v2 changes
 ...
 * Run with: bun analysis/backtest-v2.ts
 */
...
async function main() {
  ...
  for (const { wk_ms, wk_str: wk, expiration_timestamp: expIso, days_to_expiry: actualDte } of weeklyExpiryChoice) {
    ...
    // Position SIZE (contract count) is chosen the same way as v1 -- as if
    // cash-securing $100K/3 notional per leg -- so v2 sells the SAME real
    // options in the SAME quantity as v1. What changes is how much CAPITAL
    // that position ties up: v1 locked the full notional (strike*amount);
    // v2 locks Deribit's actual initial-margin requirement for that same
    // position (see file header for the formula/source). This isolates the
    // capital-sizing variable cleanly -- premium collected per leg is
    // identical to v1's; only capitalDeployedUsd (and therefore premiumPct,
    // pnlPct, and how many weeks are skippable for insufficient capital)
    // changes.
    const targetPerLegNotionalUsd = INITIAL_CAPITAL / LADDER_TARGETS.length;
    const futuresPriceAtEntry = legRows[0]!.futures_price;
    const legs: Leg[] = legRows.map(r => {
      const amount = targetPerLegNotionalUsd / r.strike;
      ...
      // IM = [max(0.15-OTM%,0.10) + option_mark_price] * underlying * amount
      // -- both the margin-rate term and option_mark_price are already
      // fractions of the underlying (BTC-denominated, matching Deribit's
      // coin-margined convention), so their sum times futures_price*amount
      // converts the whole thing to USD.
      const marginUsd = (marginRateOfUnderlying + r.mark_price) * futuresPriceAtEntry * amount;
      return {
